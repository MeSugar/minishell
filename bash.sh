# dquotes
# echo dquotes: ;  echo "aaa\bbb"ccc"ddd" ; echo "aaa\'bbb"ccc"ddd" ; echo "aaa\"bbb"ccc"ddd" ; echo "aaa'bbb"ccc"ddd" ; echo "aaa"bbb"ccc"ddd ; echo ---

# quotes
# echo quotes: ;  echo 'aaa\bbb'ccc'ddd' ; echo 'aaa\"bbb'ccc'ddd' ; echo 'aaa\"bbb'ccc'ddd' ; echo 'aaa"bbb'ccc'ddd' ; echo 'aaa'bbb'ccc'ddd ; echo ---

# backslash
# echo backslash1: ;  echo 'aaa\\bbb'ccc'ddd' ; echo 'aaa\"\bbb\"ccc'ddd ; echo 'aaa\"bbb'\\ccc'ddd' ; echo \'aaa\"bbb'ccc\'ddd ; echo \'aaa\'\bbb'cc\c\'ddd ; echo ---
# echo backslash2: ;  echo "aaa\\bbb"ccc"ddd" ; echo "aaa\'\bbb\"ccc"ddd ; echo "aaa\'bbb"\\ccc"ddd" ; echo \"aaa\'bbb"ccc\"ddd" ; echo \"aaa\"\bbb"cc\c\"ddd" ; echo ---

